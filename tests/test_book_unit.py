...
"""
Файл содержит интеграционные тесты через HTTP к запущенному сервису (BASE_URL, по умолчанию http://localhost:8000). 
Тесты проверяют работу API как черного ящика: отправляют запросы, ожидают корректные HTTP-коды и JSON-ответы, подтверждая совместную работу FastAPI и базы данных.
Покрытие по модулям:
Smoke: GET / возвращает 200 и message ok.
Справочники: CRUD для книг, филиалов, факультетов (создание, получение/список, обновление, удаление).
Администрирование: установка количества экземпляров книги в филиале (admin/stock) и создание/удаление связи книга–факультет (admin/book-faculty).
Аналитика: количество экземпляров книги в филиале и список факультетов, использующих книгу (при наличии экземпляров в филиале).
Обработка ошибок: 409 при дубликатах записей и запретах удаления (книга при остатках в филиале, филиал при остатках, факультет при наличии связи).
"""
